...
public class SimulationRuleHelper {
    ...
    private static SimulationRuleBean createSimulationRule(SimulationData.Data data) {
        SimulationRuleBean rule = new SimulationRuleBean(data.getName(), data.getSourceVertex().getDataModel(), data.getDestinationVertex().getDataModel(), data.getPacketCount(), data.getPacketSize(), data.getActivationDelay(), data.getLayer4protocol(), data.getIpPrecedence(),data.getSrcPort(),data.getDestPort());
        return rule;
    }
