...
public class ImageResourceHelper {
     ...
     * this image will be used to display vertex
     * <p/>
     * this method uses cache, so that vertex image is loaded into memory just once
     * <p/>
     ...
     */
    public static Icon loadImageVertex(ImageType imgType) {
        if (! cache.containsKey(new ImageKey(imgType.getResourcePath(), true, 0, 0))) {
            cache.put(new ImageKey(imgType.getResourcePath(), true, 0, 0), loadImage(imgType.getResourcePath()));
        }
        return new LayeredIcon(cache.get(new ImageKey(imgType.getResourcePath(), true, 0, 0)).getImage());
    }
    ...
    public static ImageIcon loadImage(String resourcePath) {
        ...
        if (url == null) throw new NullPointerException("resource file not found");
