...
public class CloseableTabbedPane extends JTabbedPane implements MouseListener,
    ...
    private static final long serialVersionUID = -8186688121559480246L;
    /**
...
//        setFont(myFont);
    ...
    protected boolean fireCloseTab(int tabIndexToClose) {
        ...
        for (int i = 0; i < listeners.length; i++) {
            Object comp = listeners[i];
            if (comp instanceof CloseableTabbedPaneListener) {
                if (!((CloseableTabbedPaneListener) comp).closeTab(tabIndexToClose)) {
                    closeit = false;
                    break;
                }
            }
            ...
            if (doPaintCloseIcon) {
                ...
                if (normalCloseIcon != null && !mouseover) {
                    ...
                    if (mousepressed && mouseover) {
                        drawCross(g2, x_pos + 1, y_pos + 1, 10, 10);
                    } else {
                        drawCross(g2, x_pos, y_pos, 11, 11);
                    }

                    g2.setColor(col);
                    if (fileIcon != null) {
                        fileIcon.paintIcon(c, g2, x + width, y_p);
                    }
                }
            }
        }

        private void drawCross(Graphics2D g2, int x, int y, int width, int height) {
            Color col = g2.getColor();
            g2.setColor(Color.BLACK);
            //draw shadow
//            g2.drawLine(x + 4, y + 5, x + width, y + height);
//            g2.drawLine(x + width, y + 5, x + 4, y + height);
            //draw with original color
            g2.setColor(col);
            g2.drawLine(x + 5, y + 6, x + width, y + height);
            g2.drawLine(x + width, y + 6, x + 5, y + height);
        }
